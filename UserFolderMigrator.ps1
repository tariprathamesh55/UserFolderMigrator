...
if ($_pluginFiles.Count -gt 0) {
    ...
    if ($_manifestMode -eq 'open') {
        Write-Host '  [i] No manifest found — running in unsigned mode. Run New-PluginManifest.ps1 to enable signing.' -ForegroundColor DarkYellow
    }
...
function Invoke-PluginInputCollection {
    ...
    if ($Unattended) {
        Write-Host '  [i] Unattended mode — using plugin defaults' -ForegroundColor DarkYellow
    }
